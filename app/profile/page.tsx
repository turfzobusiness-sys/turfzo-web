...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"bookings" | "reviews" | "favourites" | "payments" | "settings">("bookings");
        ...
        <div className="max-w-6xl mx-auto px-5 md:px-8 w-full">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "reviews" && (
                <div className="bg-surface/50 border border-border-default rounded-lg p-8 sm:p-12 text-center max-w-2xl mx-auto flex flex-col items-center">
                  ...
                  <h3 className="font-bold text-text-main text-lg mb-2">No reviews written yet</h3>
                  <p className="text-sm text-text-muted mb-6 leading-relaxed">
                    You haven't shared feedback on any turfs. Reviewing venues helps fellow athletes in your city find the best sports facilities.
                  </p>
                  <Link href="/explore" className="inline-flex items-center gap-2 px-6 py-3 bg-brand-btn-bg border border-brand-lime/30 text-white hover:border-brand-lime/60 hover:bg-brand-btn-bg-hover font-semibold text-sm rounded-lg transition-all">
              ...
              {activeTab === "favourites" && (
                <div className="bg-surface/50 border border-border-default rounded-lg p-8 sm:p-12 text-center max-w-2xl mx-auto flex flex-col items-center">
                  ...
                  <h3 className="font-bold text-text-main text-lg mb-2">Your Favourites list is empty</h3>
                  <p className="text-sm text-text-muted mb-6 leading-relaxed">
                    Save your go-to football turfs, cricket nets, or tennis courts by tapping the heart icon on venue pages to see them here.
                  </p>
                  <Link href="/explore" className="inline-flex items-center gap-2 px-6 py-3 bg-brand-btn-bg border border-brand-lime/30 text-white hover:border-brand-lime/60 hover:bg-brand-btn-bg-hover font-semibold text-sm rounded-lg transition-all">
              ...
              {activeTab === "payments" && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  <div className="lg:col-span-7 bg-surface/50 border border-border-default rounded-lg p-6 sm:p-8 flex flex-col gap-6">
                    <div>
                      <h3 className="font-bold text-text-main text-lg">Billing & Payment</h3>
                      <p className="text-xs text-text-muted mt-1">Manage your saved cards and payment options</p>
                    </div>

                    <div className="relative overflow-hidden rounded-xl bg-elevated border border-border-strong p-6 text-left h-44 flex flex-col justify-between shadow-card-shadow">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="text-[10px] text-text-muted uppercase tracking-widest font-sans font-bold">Turfzo Billing Profile</span>
                          <h4 className="text-sm font-semibold text-text-main mt-0.5">{displayName}</h4>
                        </div>
                        <CreditCard className="w-7 h-7 text-brand-lime opacity-80" />
                      </div>
                      <div className="text-right">
                        <span className="text-xs text-text-muted block">Account Tier</span>
                        <span className="text-sm font-bold text-brand-lime">Standard Player</span>
                      </div>
                    </div>

                    <div className="border border-border-default bg-elevated/30 rounded-lg p-5 text-center mt-2 flex flex-col items-center gap-3">
                      <span className="text-xs font-semibold text-text-main">No Saved Cards</span>
                      <p className="text-[11px] text-text-muted leading-relaxed max-w-sm">
                        For your security, we don't store full credentials. You can opt to remember cards using our secure payment gateway during your next checkout.
                      </p>
                    </div>
                  </div>

                  <div className="lg:col-span-5 bg-surface/50 border border-border-default rounded-lg p-6 flex flex-col gap-4">
                    <h4 className="text-sm font-bold text-text-main border-b border-border-subtle pb-2">Supported Options</h4>
                    <div className="space-y-4 text-xs font-sans">
                      <div className="flex justify-between items-center">
                        <span className="text-text-muted">UPI (GPay / PhonePe / Paytm)</span>
                        <span className="text-brand-lime font-bold">Supported</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-text-muted">Net Banking (All Major Banks)</span>
                        <span className="text-brand-lime font-bold">Supported</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-text-muted">Visa / Mastercard / RuPay</span>
                        <span className="text-brand-lime font-bold">Supported</span>
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === "settings" && (
                <div className="max-w-3xl bg-surface/50 border border-border-default rounded-lg p-6 sm:p-8 flex flex-col gap-8">
                  <div>
                    <h3 className="font-bold text-text-main text-lg">Account Settings</h3>
                    <p className="text-xs text-text-muted mt-1">Configure security features and notification settings</p>
                  </div>

                  <div className="flex flex-col gap-4 border-b border-border-subtle pb-6">
                    <h4 className="text-sm font-bold text-text-main flex items-center gap-2">
                      <Bell className="w-4 h-4 text-text-muted" /> Notification Preferences
                    </h4>
                    <div className="space-y-4 pt-2">
                      <div className="flex justify-between items-center">
                        <div className="flex flex-col">
                          <span className="text-xs font-semibold text-text-main">Email Updates</span>
                          <span className="text-[10px] text-text-muted mt-0.5">Receive receipts and tournament confirmation emails</span>
                        </div>
                        <input type="checkbox" defaultChecked className="w-9 h-5 rounded-full bg-elevated border-border-default accent-brand-lime cursor-pointer" />
                      </div>
                      <div className="flex justify-between items-center">
                        <div className="flex flex-col">
                          <span className="text-xs font-semibold text-text-main">SMS alerts</span>
                          <span className="text-[10px] text-text-muted mt-0.5">Get booking reminders and matching PIN alerts 1 hour before slots</span>
                        </div>
                        <input type="checkbox" defaultChecked className="w-9 h-5 rounded-full bg-elevated border-border-default accent-brand-lime cursor-pointer" />
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col gap-4 border-b border-border-subtle pb-6">
                    <h4 className="text-sm font-bold text-text-main flex items-center gap-2">
                      <Shield className="w-4 h-4 text-text-muted" /> Security & Access
                    </h4>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                      <div className="flex flex-col">
                        <span className="text-xs font-semibold text-text-main">Reset Password</span>
                        <span className="text-[10px] text-text-muted mt-0.5">Triggers a password reset instruction link to {convexUser?.email ?? firebaseUser?.email}</span>
                      </div>
                      <button 
                        onClick={() => {
                          toast.success("Password reset email sent!");
                        }}
                        className="px-4 py-2 border border-border-default hover:border-brand-lime/30 text-xs font-semibold rounded-lg bg-surface text-text-main transition-colors"
                      >
                        Reset Password
                      </button>
                    </div>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                      <div className="flex flex-col">
                        <span className="text-xs font-semibold text-text-main">Delete Account</span>
                        <span className="text-[10px] text-text-muted mt-0.5">Permanently delete your profile and cancel all active bookings. This action is irreversible.</span>
                      </div>
                      <button 
                        onClick={() => {
                          const confirm = window.confirm("Are you absolutely sure you want to delete your account? This will cancel all bookings and remove your data.");
                          if (confirm) {
                            toast.success("Account deletion request submitted.");
                          }
                        }}
                        className="px-4 py-2 border border-error/30 hover:bg-error/10 text-xs font-semibold rounded-lg text-error transition-colors"
                      >
                        Delete Profile
                      </button>
                    </div>
