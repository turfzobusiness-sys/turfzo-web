...
export default function BookingsPage() {
      ...
      try {
        ...
        await Promise.all(
          turfIds.map(async (id) => {
            try {
              const t = await convexClient.query<Turf | null>("turfs:getById", { id });
              if (t) turfMap[id] = t;
            } catch {
  ...
  const handleCancel = async (bookingId: string) => {
    if (!confirm("Are you sure you want to cancel this booking? Refund rules apply.")) return;
    try {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    if (filter === "upcoming") return b.status === "confirmed" && new Date(b.start_time) > new Date();
    if (filter === "past") return b.status === "completed" || (b.status === "confirmed" && new Date(b.start_time) <= new Date());
    return true;
  });
                      ...
                      <div>
                        ...
                        {turf && (
                          <p className="text-xs text-text-muted font-sans flex items-center gap-1 mt-1">
                            <MapPin className="w-3 h-3 text-brand-lime" /> {turf.city ?? turf.address ?? "—"}
                          </p>
                        )}
                        <div className="flex flex-wrap items-center gap-4 mt-3 text-[11px] text-text-muted font-sans">
                          <span className="flex items-center gap-1.5">
                            ...
                            {new Date(booking.start_time).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                          </span>
                          <span className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold text-brand-lime">₹{booking.total_price.toLocaleString("en-IN")}</span>
                          <span className="text-text-muted/70">#{booking.booking_code}</span>
                        </div>
                      </div>
